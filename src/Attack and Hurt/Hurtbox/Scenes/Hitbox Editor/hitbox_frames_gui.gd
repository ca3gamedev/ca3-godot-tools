	...
	if anims != null:
		for track in anims.get_track_count():
			...
			if anims.track_get_path(track) == NodePath("../CollisionShape2D:shape:size"):
				anims.track_insert_key(track, %HitboxFramesGUI.duration, %HitboxPosGUI.Size)
			if anims.track_get_path(track) == NodePath("../CollisionShape2D:debug_color"):
				...
				if not key:
					anims.track_insert_key(track, time, Color8(1, 47, 56, 172))
				else:
					anims.track_insert_key(track, time, Color8(7, 127, 148, 172))
