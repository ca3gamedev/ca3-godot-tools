...
func GetClosestEnemy():
	...
	for i in $"../Enemies".get_children():
		if i.IsAI and i.is_in_group("ENEMY") and i.GetDistance() < minimal_distance:
...
func DistanceSort():
	...
	for i in len($"../Enemies".get_children()):
		if $"../Enemies".get_child(i) != null and $"../Enemies".get_child(i).is_in_group("ENEMY") and $"../Enemies".get_child(i).IsAI:
	...
	if len(distance_tiers) <= 4:
		...
		for i in len(distance_tiers):
			var id = int(i / 4)
			var tier = distance_tiers[i][1]
			if id < len(tiers):
				tiers[id].append(tier)
