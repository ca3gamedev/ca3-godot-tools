...
func NewAnimation(loop, pre_name, current_anim):
	...
	anim.track_set_path(frame_index, "../Hitbox/CollisionShape2D:disabled")
	anim.track_insert_key(frame_index, 0.0, true)
	anim.track_set_interpolation_type(frame_index, 0)
	
	var frame_index2 = anim.add_track(Animation.TYPE_VALUE)
	anim.track_set_path(frame_index2, "../Hitbox/CollisionShape2D:position")
	anim.track_insert_key(frame_index2, 0.0, Vector2.ZERO)
	anim.track_set_interpolation_type(frame_index2, 0)
	
	var frame_index3 = anim.add_track(Animation.TYPE_VALUE)
	anim.track_set_path(frame_index3, "../Hitbox/CollisionShape2D/DebugAttackBox:color")
	anim.track_insert_key(frame_index3, 0.0, Color8(185, 15, 64, 100))
