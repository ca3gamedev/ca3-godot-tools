...
func COINSCHECK():
	var coin_removal = []
	for i in len(coins):
		if not is_instance_valid(coins[i]):
			coin_removal.append(i)
	for i in coin_removal:
		coins.remove_at(i)
	var smallest_coin_distance = 999999
	for i in len(coins):
		var dist = $"../../".global_position.distance_to(coins[i].global_position)
		if dist < smallest_coin_distance:
			smallest_coin_distance = dist
