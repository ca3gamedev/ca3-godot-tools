...
func Save(CurrentFile):
	...
	if not save_file:
		Frame.LOG([["Failed to open file for saving!!!."], true], 8.0, false)
		return
...
func LoadSave(file):
	...
	if not FileAccess.file_exists(file_name):
		Frame.LOG([["Not save file!!!."], true], 8.0, false)
		return
	...
	if parse_result != OK:
		var log_text =  "JSON Parse Error: " + str(json.get_error_message()) + " at line " + str(json.get_error_line())
		Frame.LOG([[log_text], true], 8.0, false)
		return
