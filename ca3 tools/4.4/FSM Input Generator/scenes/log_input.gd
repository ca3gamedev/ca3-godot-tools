...
@onready var buffer = get_tree().get_root().get_node("Inputmanager/INPUT/Buffer")
...
func _process(delta):
	...
	var children = get_tree().get_root().get_node("Inputmanager/INPUT/FSM/Move").get_children()
	if children.size() > 0:
	...
	var special = str(get_tree().get_root().get_node("Inputmanager/INPUT").GetInput())
	if special != "none":
